    ...
    >
      <div className="flex p-8 flex-col lg:flex-row bg-white min-h-screen">
        {blogposts.data.map((post) => {
          ...
          var { thumbnail } =
            post.attributes.featuredImg.data.attributes.formats;
          return (
            <Link
            ...
            >
              <div className="cursor-pointer lg:w-[400px] lg:h-[300px] bg-neutral-900 rounded-2xl overflow-hidden flex flex-col">
                <Image
                  ...
                  objectFit="contain"
                ></Image>
                <h1 className="font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 via-green-200 to-green-300 text-4xl">
                  {title}
                </h1>
              </div>
            </Link>
...
export async function getStaticProps() {
  const blogposts = await fetchData(
    "/api/blogposts?fields=slug,title&populate=%2A"
  );
  return {
    props: {
      blogposts,
