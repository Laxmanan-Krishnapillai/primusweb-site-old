...
export default function Blog({blogposts}) {
  return(
    ...
    {blogposts.data.map((post)=>{
      ...
      var { alternativeText } = post.attributes.featuredImg.data.attributes;
      var { thumbnail } = post.attributes.featuredImg.data.attributes.formats;
      return (
        <Link className="cursor-pointer" href={'/posts/' + slug} passHref key={slug}>
          <div className='cursor-pointer w-[400px] h-[300px] bg-neutral-900 rounded-2xl overflow-hidden flex flex-col'>
            <Image className='w-full h-auto' src={getStrapiMedia(thumbnail.url)} width={thumbnail.width} height={thumbnail.height} objectFit="contain" alt={alternativeText}></Image>
...
export async function getStaticProps() {
    const blogposts = await fetchData("/api/blogposts?fields=description,slug,title&populate=%2A");
    return {
      props: {
        blogposts
      }
